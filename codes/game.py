...
class Game:
    def __init__(self, wsize):
        ...
        self.pool = piece_names[:]
        self.pool.remove(King)
        self.pool.remove(Pawn)

        self.promotion_mode = "fairy"
        ...
        if None not in startpos + endpos:
            ...
            if target and target.color == self.playersturn:
                ...
                if self.cannot_move(target.color, self.gameboard):
                    if self.is_check(target.color, self.gameboard):
                        self.message = f"Checkmate! {self.opponent[target.color]} player won!"
                        sys.exit()
                    else:
    ...
    def is_check(self, color, gameboard):
        ...
        if self.can_see_king(kingDict[color], pieceDict[self.opponent[color]], gameboard):
            return True
    ...
    def draw(self):
        ...
        # プロモーション
        if self.prom and self.promotion_mode == "vanilla":
        ...
        if self.prom and self.promotion_mode == "fairy":
            draw_balloon(*self.endpos)
            piece_color = self.gameboard[self.endpos].color
            glEnable(GL_TEXTURE_2D)
    ...
    def mouse(self, button, state, x, y):
        ...
        x, y : int
            ウィンドウ座標．
        '''
        self.mousepos = window2world(x, y, self.wsize)
        # 左クリック
    ...
    def glmain(self):
        ...
        glutInitWindowSize(self.wsize, self.wsize)                # 画面サイズ
